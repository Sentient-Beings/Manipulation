...
using MovingGroupInterface = moveit::planning_interface::MoveGroupInterface;

int main(int argc, char **argv)
{
    rclcpp::init(argc, argv);

    // spin the node in a separate thread since we dont want to block the main
    auto node = std::make_shared<rclcpp::Node>("test_moveit");
    rclcpp::executors::SingleThreadedExecutor executor;
    executor.add_node(node);
    auto spinner = std::thread([&executor]() { executor.spin(); });

    auto arm = MovingGroupInterface(node, "arm");
    arm.setMaxVelocityScalingFactor(.0);
    arm.setMaxAccelerationScalingFactor(1.0);

    auto gripper = MovingGroupInterface(node, "gripper");
    ...
    MovingGroupInterface::Plan plan1;
    bool success1 = (arm.plan(plan1) == moveit::core::MoveItErrorCode::SUCCESS);
